...
export default defineConfig(({ mode }) => {
    ...
    const globalApiTarget = (env.VITE_GLOBAL_API_BASE_URL || "https://global-travel.mn").replace(/\/$/, "");
    const localApiTarget = (env.VITE_LOCAL_API_TARGET || "http://localhost:8080").replace(/\/$/, "");
    return {
        ...
        build: {
            ...
            rollupOptions: {
                output: {
                    manualChunks(id) {
                        if (id.includes("/src/components/OrdersTab.tsx") ||
                        ...
                        if (id.includes("/src/components/AddPassengerTab.tsx") ||
                            id.includes("/src/components/AddTourTab.tsx")) {
                            return "manager-booking";
                        }
                        if (id.includes("/src/components/PassengerRequests.tsx") ||
