-- Shared booking core for B2B/B2C canonical writes
-- Run in Supabase SQL Editor.

begin;

-- 1) Canonical source/idempotency fields
do $$
begin
  if not exists (
    ...
    alter table public.orders add column source text not null default 'b2b';
  end if;

  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'orders' and column_name = 'source_order_id'
  ) then
    alter table public.orders add column source_order_id text;
  end if;

  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'passengers' and column_name = 'source_passenger_id'
  ) then
    alter table public.passengers add column source_passenger_id text;
  end if;
end
$$;

create unique index if not exists orders_source_source_order_id_uidx
  on public.orders (source, source_order_id)
  where source_order_id is not null;

create index if not exists orders_tour_departure_idx
  on public.orders (tour_id, "departureDate");

create index if not exists passengers_order_id_idx
  on public.passengers (order_id);

create index if not exists passengers_tour_departure_idx
  on public.passengers (tour_id, departure_date);

-- 2) Seat visibility function
create or replace function public.get_departure_seats(
  p_tour_id text,
...
begin
  ...
  select coalesce(sum(greatest(coalesce(p.seat_count, 1), 1)), 0)
  into v_booked
  from public.passengers p
  join public.orders o on o.id = p.order_id
  where o.tour_id = p_tour_id
    and o."departureDate" = p_departure_date
    and coalesce(lower(o.status), 'pending') not in ('cancelled', 'declined');

  return query
    select v_capacity, v_booked, greatest(v_capacity - v_booked, 0);
end
$$;

-- 3) Canonical transactional booking function
...
grant execute on function public.get_departure_seats(text, text) to anon, authenticated, service_role;
grant execute on function public.book_trip_shared(text, text, text, text, text, text, text, text, jsonb) to authenticated, service_role;
