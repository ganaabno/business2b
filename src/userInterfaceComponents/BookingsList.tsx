...
import type { Tour, Passenger, Order, User as UserType } from "../types/type";
...
export default function BookingsList({
  ...
  useEffect(() => {
    ...
    // ✅ Cleanup is now sync — doesn't return a Promise
    return () => {
  ...
  const totalPages = Math.ceil((sortedPassengers.length + sortedPendingRequests.length) / itemsPerPage);
  const paginatedItems = [...sortedPendingRequests, ...sortedPassengers].slice(
  ...
  return (sortedPassengers.length > 0 || sortedPendingRequests.length > 0) ? (
    <div className="mt-8 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-8 py-6 border-b border-gray-100">
        ...
        <p className="text-sm text-gray-600 mt-1">Manage and view your travel reservations and pending requests</p>
      </div>
      <div className="p-8">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-semibold text-gray-900">{item.tour_title}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Departure</p>
                  <p className="text-sm text-gray-700">{formatDisplayDate(item.departure_date || "")}</p>
                </div>
                <div className="space-y-1">
