  ...
  const handleRemoveFromBlacklist = async (passengerId: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        showNotification(
          "error",
          `Failed to remove passenger from blacklist: ${error.message}`,
        );
      ...
      console.error(
        "Unexpected error removing passenger from blacklist:",
        error,
      );
      showNotification(
        "error",
        "An unexpected error occurred while removing passenger from blacklist.",
      );
      setPassengers(previousPassengers); // Rollback on error
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {filteredAndSortedPassengers.length} of{" "}
              {passengers.filter((p) => p.is_blacklisted).length} blacklisted
              passengers
            </p>
          </div>
