...
import type { Tour, User as UserType, Notification as NotificationType, LeadPassenger } from "../types/type";
...
interface LeadPassengerFormProps {
  ...
  setLeadPassengerData: React.Dispatch<React.SetStateAction<LeadPassenger | null>>;
...
export default function LeadPassengerForm({
  ...
  const isPowerUser = ["admin", "manager", "superadmin"].includes(currentUser.role || "");
  const maxSeats = isPowerUser ? 100 : selectedTourData?.available_seats ?? 10;

  const validateForm = useCallback(() => {
    const newErrors: Record<string, string> = {};
    if (!leadPassenger.first_name.trim()) newErrors.first_name = "First name is required";
    if (!leadPassenger.last_name.trim()) newErrors.last_name = "Last name is required";
    if (!leadPassenger.phone.trim() || !/^[0-9]{4}$/.test(leadPassenger.phone.replace(/\D/g, ""))) {
    ...
    if (isNaN(seatCountNum) || seatCountNum < 1) newErrors.seat_count = "At least one seat is required";
    if (!isPowerUser && selectedTourData?.available_seats && seatCountNum > selectedTourData.available_seats) {
  ...
  const checkDuplicateLead = async (): Promise<boolean> => {
    ...
    if (error) {
      ...
      setNotification({ type: "error", message: `Failed to check duplicates: ${error.message}` });
      return false;
    }

    if (data && data.length > 0) {
      setNotification({ type: "error", message: "This lead passenger is already registered with the same details." });
      return true;
    }
    return false;
  };

  const handleInputChange = (field: keyof typeof leadPassenger, value: string | number) => {
  ...
  const startTimer = useCallback((expiresAt: string) => {
    ...
    const updateTimer = () => {
      ...
      const timeRemaining = Math.max(0, Math.floor((expiryDate - now) / 1000));
      setTimeLeft(timeRemaining);

      if (timeRemaining <= 0) {
        setLeadPassengerData(null);
        setLeadId(null);
        setNotification({ type: "error", message: "Lead reservation expired. Please try again." });
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedTourData || !selectedTour || !departureDate) {
      setNotification({ type: "error", message: "No tour or departure date selected" });
      return;
    }
    ...
    try {
      ...
      setNotification({
        type: "error",
        message: error instanceof Error ? error.message : "Failed to register lead passenger",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <div className="sticky top-0 z-10 bg-gradient-to-r from-slate-50 to-blue-50 rounded-xl shadow-sm border border-slate-200 mb-6">
        <div className="px-6 py-4 border-b border-slate-200">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Lead Passenger Information</h3>
              <p className="text-sm text-slate-600">
                Tour: {selectedTourData?.title || "Not selected"} | {calculateLeadExpiryHours(departureDate)} hour
                {calculateLeadExpiryHours(departureDate) !== 1 ? "s" : ""} to complete
              </p>
            </div>
          </div>
        </div>
      ...
      {timeLeft !== null && (
        <div className="mb-6 p-3 bg-yellow-100 text-yellow-800 rounded-lg flex items-center">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-md">
        <div>
          ...
          <input
            ...
            className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm ${errors.first_name ? 'border-red-500' : ''}`}
        ...
        <div>
          ...
          <input
            ...
            className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm ${errors.last_name ? 'border-red-500' : ''}`}
            disabled={loading || !!leadId}
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => handleInputChange("seat_count", e.target.value)}
            className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm ${errors.seat_count ? 'border-red-500' : ''}`}
            disabled={loading || !!leadId}
            placeholder="Enter seats (1-10)"
          />
          {errors.seat_count && <p className="text-red-500 text-sm mt-1">{errors.seat_count}</p>}
        </div>
