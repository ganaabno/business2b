    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 md:hidden z-40">
      <div className="flex justify-between items-center mb-2">
        <div className="text-sm text-gray-600">
          {t("managerStepOf", { step: Math.min(activeStep, 4), total: 4 })}
        </div>
        <div className="text-sm font-medium text-gray-900">
          {bookingPassengers.length > 0 && (
            <span>
              {bookingPassengers.length} {bookingPassengers.length === 1 ? t("passengerSingular") : t("passengerPlural")} • ${totalPrice.toLocaleString()}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        {activeStep < 3 && (
          ...
          >
            {activeStep === 1
              ? t("mobileFooterContinueToLead")
              : t("mobileFooterContinueToPassengers")}
          </button>
        )}
      </div>
    </div>
