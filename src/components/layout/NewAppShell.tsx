...
import {
  LayoutDashboard, Users, Plane, ShoppingCart, Settings,
  Menu, X, ChevronLeft, Bell, Search, LogOut, 
  CheckCircle, Ticket, UserPlus, FileText, Building2, 
  PhoneIncoming, Ban, PlaneTakeoff, MessageCircle, ClipboardList
} from "lucide-react"

  const managerNavItems = [
    { key: "dashboard", label: "Янах самбар", path: "/manager" },
    { key: "tasks", label: "Tasks", path: "/manager/tasks" },
    { key: "yourRequests", label: "Таны хүсэлтүүд", path: "/manager/yourRequests" },
    { key: "orders", label: "Захиалгууд", path: "/manager/orders", badge: 100 },
    { key: "passengers", label: "Бүх зорчигч", path: "/manager/passengers", badge: 290 },
    { key: "addPassenger", label: "Зорчигч бүртгэх", path: "/manager/add-passenger" },
    ...
    { key: "providerAssignments", label: "Провайдер эрх", path: "/manager/provider-assignments" },
    { key: "interestedLeads", label: "Сонирхсон lead", path: "/manager/interested-leads" },
    { key: "passengerRequests", label: "Зорчигчийн хүсэлт", path: "/manager/passenger-requests" },
    { key: "pendingLeads", label: "Lead зорчигч", path: "/manager/pending-leads", badge: 4 },
    { key: "blacklist", label: "Хар жагсаалт", path: "/manager/blacklist" },
    { key: "flightData", label: "Нислэгийн мэдээлэл", path: "/manager/flight-data" },
    { key: "ProviderInterface", label: "Провайдер харагдац", path: "/manager/provider-view" },
...
export function AppShell({ children }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--mono-bg)] flex">
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-[var(--mono-border)]">
          <Link to="/" className="flex items-center gap-3">
            ...
            {isOpen && (
              <span className="font-bold text-lg text-[var(--mono-text)]">GTrip</span>
            )}
          </Link>
          <button
          ...
          >
            <ChevronLeft className={cn("w-5 h-5 text-[var(--mono-text-soft)] transition", !isOpen && "rotate-180")} />
          </button>
        </div>
        ...
        <div className="p-3 border-t border-[var(--mono-border)]">
          <div className={cn("flex items-center gap-3 p-2 rounded-xl hover:bg-[var(--mono-surface-muted)] transition", !isOpen && "justify-center")}>
            ...
            {isOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[var(--mono-text)] truncate">{currentUser?.email}</p>
                <p className="text-xs text-[var(--mono-text-soft)] capitalize">{role}</p>
              </div>
            )}
            {isOpen && (
              <button
        ...
        <>
          ...
          <aside className="fixed left-0 top-0 h-screen w-72 bg-[var(--mono-surface)] border-r border-[var(--mono-border)] z-50 lg:hidden">
            <div className="h-16 flex items-center justify-between px-4 border-b border-[var(--mono-border)]">
              ...
              <button onClick={() => setMobileOpen(false)} className="p-2 rounded-lg hover:bg-[var(--mono-surface-muted)]">
                <X className="w-5 h-5" />
              </button>
            </div>
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive ? "bg-[var(--mono-accent-soft)] text-[var(--mono-accent)] font-medium" : "text-[var(--mono-text-soft)]"
                    )}
                  >
      ...
      {/* Main content */}
      <main className={cn("flex-1 flex flex-col min-h-screen transition-all duration-300", isOpen ? "lg:ml-64" : "lg:ml-20")}>
        {/* Top bar */}
        <header className="sticky top-0 z-30 h-16 bg-[var(--mono-surface)] border-b border-[var(--mono-border)] flex items-center justify-between px-4 lg:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/change-password" className="p-2 rounded-lg hover:bg-[var(--mono-surface-muted)]">
