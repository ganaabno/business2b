...
import { resolveUserRoleFromProfile } from "../utils/roles";
...
function normalizeUserRow(row: any): User | null {
  ...
  const resolvedRole = resolveUserRoleFromProfile({
...
function RoleChanger({ users, setUsers, currentUser }: RoleChangerProps) {
  ...
  const fetchUsers = async () => {
    ...
    if (mergedUsers.length === 0 && fallbackPlain) {
      const primaryMessage = String(
        (listError as { message?: unknown } | null)?.message || "",
      ).toLowerCase();

      if (primaryMessage.includes("missing auth session")) {
  ...
  const handleChangeRole = async (
    ...
    const normalizedCurrentRole = String(currentRole || "")
      .trim()
      .toLowerCase();
    const normalizedNewRole = String(newRole || "")
      .trim()
      .toLowerCase();
    ...
    if (isSelfRoleChange) {
      const confirmed = window.confirm(
        `You are changing your own role from ${normalizedCurrentRole} to ${normalizedNewRole}. This can immediately remove your current admin access. Continue?`,
      );
  ...
  const filteredUsers = displayUsers.filter((user) => {
    ...
    return normalizedRole === selectedRoleFilter;
  });
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={(e) => {
              setSelectedRoleFilter(e.target.value as RoleFilter);
              setCurrentPage(1);
            }}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    >
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <select
                          ...
                          disabled={loading[user.id] || isPendingRequest}
                          onChange={(e) =>
                            handleChangeRole(
                              user.id,
                              e.target.value,
                          ...
                          className={`w-full max-w-[100px] px-2 py-1 text-sm border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                            loading[user.id] || isPendingRequest
        ...
        <div className="border-t border-gray-200 px-4 py-3 flex items-center justify-between text-sm text-gray-600">
          <p>
            ...
            {Math.min(startIndex + paginatedUsers.length, filteredUsers.length)} of {filteredUsers.length}
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
