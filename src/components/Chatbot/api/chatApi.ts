import type { ChatApiResponse, ChatContext, ChatMessage, TourForAI } from '../types';
import { supabase } from '../../../supabaseClient';

const API_BASE_URL = import.meta.env.VITE_BASE_URL || 'https://gtrip.mn';

class ChatApiError extends Error {
  constructor(public message: string, public code?: string, public retryable = true) {
    super(message);
...
export async function extractBookingDetails(message: string): Promise<Record<string, unknown>> {
  try {
    const token = await getAuthToken();
    ...
    if (!response.ok) {
      throw new ChatApiError('Failed to extract details', 'EXTRACT_ERROR', true);
    }
...
export async function getGreeting(userPreferences?: Record<string, unknown>): Promise<string> {
  try {
    const token = await getAuthToken();
...
export async function fetchToursForAI(limit = 20): Promise<TourForAI[]> {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/chat/tours?limit=${limit}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
...
export async function fetchCheapestToursForAI(count = 5): Promise<TourForAI[]> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/chat/tours/cheapest?count=${count}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      throw new ChatApiError('Failed to fetch cheapest tours', 'TOURS_ERROR', true);
    }
...
export async function searchToursAI(destination: string): Promise<TourForAI[]> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/chat/tours/search?destination=${encodeURIComponent(destination)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
