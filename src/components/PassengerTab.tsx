...
const validateField = (
  ...
  switch (field) {
    ...
    case "date_of_birth":
      ...
      return null;
    case "gender":
      return !passenger.gender || passenger.gender === ""
        ? "Gender is required"
        : null;
    case "passport_number":
      return !passenger.passport_number || passenger.passport_number === ""
    ...
    case "passport_expire":
      ...
      return null;
    case "nationality":
      return !passenger.nationality || passenger.nationality === ""
        ? "Nationality is required"
        : null;
    case "hotel":
      return !passenger.hotel || passenger.hotel === ""
        ? "Hotel is required"
        : null;
    case "status":
      return !passenger.status || passenger.status === "pending"
        ? "Status is required"
        : null;
    case "notes":
...
export default function PassengersTab({
  ...
  const [passengerStatusFilter, setPassengerStatusFilter] =
    useState<string>("all");
  const [passengerDepartureFilter, setPassengerDepartureFilter] =
    useState<string>("");
  ...
  const handlePassengerChange = (
    ...
    if (isEditMode) {
      ...
      const error = validateField(
        field,
        value,
        updatedPassengers.find((p) => p.id === id)!,
      );
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (hasError) {
          ...
          showNotification(
            "error",
            `Failed to update passengers: ${error?.message || "Unknown error"}`,
          );
        ...
        showNotification(
          "error",
          "An unexpected error occurred while updating passengers.",
        );
        setPassengers(previousPassengers);
      } finally {
        setIsSaving(false);
  ...
  const handleDeletePassenger = async (id: string) => {
    const deletedPassenger = passengers.find(
      (passenger) => passenger.id === id,
    );

    const syncGlobalSeatsAfterDelete = async () => {
      const localTourId = String(deletedPassenger?.tour_id || "").trim();
      const departureDate = String(deletedPassenger?.departure_date || "")
        .trim()
        .slice(0, 10);
      ...
      try {
        ...
        console.warn("PassengerTab: Global seat sync failed after delete", {
          localTourId,
          departureDate,
          error,
        });
    ...
    try {
      ...
      if (error) {
        ...
        await syncGlobalSeatsAfterDelete();
  ...
  const handleBlacklistToggle = async (id: string, isChecked: boolean) => {
    ...
    try {
      ...
      if (error) {
        showNotification(
          "error",
          `Failed to update blacklist: ${error.message}`,
        );
        setPassengers(previousPassengers);
      } else {
        showNotification(
          ...
          isChecked
            ...
            : "Passenger unblacklisted! ✅",
        );
      }
    } catch (error) {
      showNotification(
        "error",
        "An unexpected error occurred while updating blacklist.",
      );
      setPassengers(previousPassengers);
    }
  };
    ...
    return [...passengers]
      .sort((a, b) => {
        const dateA = a.departure_date
          ? new Date(a.departure_date)
          : new Date(0);
        const dateB = b.departure_date
          ? new Date(b.departure_date)
          : new Date(0);
        return dateA.getTime() - dateB.getTime();
      })
      .filter((passenger) => {
        const matchesName = `${passenger.first_name || ""} ${
          passenger.last_name || ""
        }`
          .toLowerCase()
          .includes(passengerNameFilter.toLowerCase());
        const orderIdString =
          passenger.order_id != null ? String(passenger.order_id) : "";
        const matchesOrder = orderIdString
          .toLowerCase()
          .includes(passengerOrderFilter.toLowerCase());
        const matchesStatus =
          passengerStatusFilter === "all" ||
          passenger.status === passengerStatusFilter ||
          (!passenger.status && passengerStatusFilter === "active");
        const matchesDeparture =
          !passengerDepartureFilter ||
          passenger.departure_date === passengerDepartureFilter;
        return (
          matchesName &&
          matchesOrder &&
          matchesStatus &&
          matchesDeparture &&
          !passenger.is_blacklisted
        );
      });
  }, [
    passengers,
    passengerNameFilter,
    passengerOrderFilter,
  ...
  return (
    <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border border-gray-100 overflow-hidden relative backdrop-blur-sm">
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          ...
          {isEditMode && (
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 flex items-center justify-center space-x-2 shadow-lg ${
                hasValidationErrors()
      ...
      <div className="overflow-x-auto bg-white/50 backdrop-blur-sm">
        <table className="min-w-full mono-table mono-table--compact divide-y divide-gray-200/50">
          <thead className="bg-gradient-to-r from-gray-50 to-gray-100 backdrop-blur-sm">
            <tr>
              ...
              <th className="px-16 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                Tour
              </th>
              ...
              <th className="px-14 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                Gender
              </th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                Passport
              ...
              <th className="px-28 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                Notes
              </th>
              <th className="px-18 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                Status
              </th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                Blacklist
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900">
                    {isEditMode ? (
                      <input
                        ...
                        className={getInputClass(
                          "departure_date" as keyof Passenger,
                          passenger,
                          isEditMode,
                        )}
                      />
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          passenger.roomType
                            ?.toLowerCase()
                            .includes("deluxe") ||
                          passenger.roomType?.toLowerCase().includes("suite")
        ...
        <div className="bg-gradient-to-r from-gray-50 to-white p-6 border-t border-gray-200">
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-600">
              ...
              to{" "}
              <span className="font-semibold">
                {Math.min(currentPage * 10, filteredPassengers.length)}
              </span>{" "}
              of{" "}
              <span className="font-semibold">{filteredPassengers.length}</span>{" "}
              results
            </div>
            <div className="flex items-center space-x-3">
              <button
      ...
      {showDeleteConfirm && (
        <ConfirmModal
          ...
          description="Are you sure you want to delete this passenger? This action cannot be undone."
          confirmLabel="Delete"
          cancelLabel="Cancel"
          onConfirm={() => showDeleteConfirm && handleDeletePassenger(showDeleteConfirm)}
          variant="danger"
        />
      )}
    </div>
