    ...
    return {
        ...
        notes: "",
        seat_count: 1, // Default seat count to 1
    };
    };
    ...
    const addPassenger = useCallback(async () => {
        ...
        if (bookingPassengers.length >= MAX_PASSENGERS) {
            showNotification("error", `Maximum ${MAX_PASSENGERS} passengers allowed per booking`);
        } else {
            showNotification("error", "Cannot add passenger. Tour is fully booked or invalid.");
        }
        return;
        }
        ...
        setPassengers((prev) => [
            ...prev.filter((p) => p.order_id !== "" || !bookingPassengers.some((bp) => bp.id === p.id)),
    ...
    }, [bookingPassengers, currentUser, selectedTourData, departureDate, showNotification, setPassengers, isAdding]);

    const addMultiplePassengers = useCallback(
        async (count: number) => {
        if (isAdding || !canAddPassenger() || bookingPassengers.length + count > MAX_PASSENGERS) {
            showNotification("error", `Cannot add ${count} passengers. Maximum ${MAX_PASSENGERS} allowed.`);
            return;
        }
        if (selectedTourData?.seats !== undefined && bookingPassengers.length + count > selectedTourData.seats) {
            showNotification("error", "Cannot add passengers. Tour is fully booked.");
            return;
        }
    ...
    const removePassenger = useCallback(
        ...
        if (bookingPassengers.length === 1 && passengers[passengerIndex].order_id === "") {
            showNotification("error", "At least one passenger is required for a new booking");
            return;
        }
        ...
        [bookingPassengers, passengers, expandedPassengerId, currentUser.userId, selectedTourData, departureDate, showNotification, setPassengers]
    );
    ...
    const handleUploadCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setExpandedPassengerId(newPassengers[newPassengers.length - 1].id);
            showNotification("success", `${newPassengers.length} passengers imported from CSV`);
            newPassengerRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
