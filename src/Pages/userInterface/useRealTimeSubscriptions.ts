...
import { checkSeatLimit } from "../../utils/seatLimitChecks"; // Ensure this is imported
...
interface UseRealTimeSubscriptionsProps {
  ...
  passengers: Passenger[]; // Added passengers prop
}
...
export default function useRealTimeSubscriptions({
              ...
              return [
                ...
                ...data
                  .map((p: SupabasePassenger): Passenger => ({
                    ...
                    seat_count: (p as any).seat_count || 1, // Default to 1 if not provided
                  }))
          ...
          try {
            ...
            setOrders(
              data.map((o): Order => ({
                ...
                passengers: passengers.filter((p) => p.order_id === o.id), // Now using the passed passengers prop
              }))
            );
            if (selectedTourData?.id && departureDate) {
              const { isValid, message } = await checkSeatLimit(selectedTourData.id, departureDate);
  ...
  }, [currentUser.userId, setPassengers, setOrders, setTours, selectedTour, departureDate, wrappedShowNotification, tours, passengers]);
}