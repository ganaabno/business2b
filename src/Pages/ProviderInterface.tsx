...
type ProviderScopeMode = "legacy" | "assigned";
type ProviderTabId =
  | "dashboard"
  | "orders"
  | "booking"
...
interface TourProviderAssignmentRow {
  tour_id: string;
}
...
function ProviderInterface({
  ...
  const isScopedProvider =
    !readOnlyPreview && ["provider", "agent"].includes(normalizedRole);
  const [providerScopeMode, setProviderScopeMode] =
    useState<ProviderScopeMode>("legacy");
  const [providerAssignedTourIds, setProviderAssignedTourIds] = useState<
    string[]
  >([]);
  const [providerScopeReady, setProviderScopeReady] = useState(false);
  const providerAssignedTourIdsKey = useMemo(
    () => providerAssignedTourIds.slice().sort().join(","),
    [providerAssignedTourIds],
  );
  const providerAssignedTourSet = useMemo(
    () => new Set(providerAssignedTourIds),
    [providerAssignedTourIds],
  );
  const canManageTours =
    !readOnlyPreview &&
    ["admin", "superadmin", "manager"].includes(normalizedRole);
  ...
  const loadProviderScope = async () => {
    if (!isScopedProvider) {
      setProviderScopeMode("legacy");
      setProviderAssignedTourIds([]);
      setProviderScopeReady(true);
      return;
    }

    try {
      const { data: assignmentsEnabled, error: enabledError } =
        await fetchWithRetry(async () =>
          supabase.rpc("fn_tour_provider_assignments_enabled"),
        );

      if (enabledError) {
        const code = String((enabledError as any)?.code ?? "");
      ...
      const assignedIds = Array.from(
        new Set(
          ((assignmentRows || []) as TourProviderAssignmentRow[])
            .map((row) => String(row.tour_id || "").trim())
            .filter(Boolean),
        ),
      );

      setProviderScopeMode("assigned");
      setProviderAssignedTourIds(assignedIds);
    } catch (error) {
      console.warn("Failed to load provider assignment scope", error);
      setProviderScopeMode("legacy");
      setProviderAssignedTourIds([]);
    } finally {
      setProviderScopeReady(true);
  ...
  const fetchOrders = async () => {
    if (hasShowInProviderOrders === null || !providerScopeReady) return;
...
`;
    ...
    if (providerScopeMode === "assigned") {
      query = query.in("tour_id", providerAssignedTourIds);
    } else if (hasShowInProviderOrders) {
    ...
    try {
      ...
      if (providerScopeMode === "assigned") {
        fallbackQuery.in("tour_id", providerAssignedTourIds);
      } else if (hasShowInProviderOrders) {
  ...
  useEffect(() => {
    ...
    const setupSubscriptions = () => {
      ...
      statuses.forEach((status) => {
        const applyLegacyShowFilter =
          providerScopeMode !== "assigned" && hasShowInProviderOrders;
        const filter = applyLegacyShowFilter
            ...
            (payload) => {
              if (providerScopeMode === "assigned") {
                ...
                const scopeTourId = String(
                  payloadAny.new?.tour_id ?? payloadAny.old?.tour_id ?? "",
                );
                const inAssignedScope =
                  !!scopeTourId && providerAssignedTourSet.has(scopeTourId);
                ...
                if (!inAssignedScope) {
                  if (
                    ...
                    const hiddenOrderId = String(
                      payloadAny.new?.id ?? payloadAny.old?.id ?? "",
                    );
                    if (hiddenOrderId) {
                      setOrders((prev) =>
                        prev.filter((order) => order.id !== hiddenOrderId),
                      );
                    }
                  }
                  return;
      ...
      const tourSubscription = supabase
        ...
        .on(
          "postgres_changes" as any,
          {
            ...
            `,
            filter:
              providerScopeMode !== "assigned" && hasShowInProviderTours
  ...
  const updateOrderStatus = async (orderId: string, status: OrderStatus) => {
    ...
    const syncGlobalSeatsForOrder = async (order: Order | undefined) => {
      if (!order) return;

      const localTourId = String(order.tour_id || "").trim();
      const departureDate = normalizeDateOnly(order.departureDate || null);
      if (!localTourId || !/^\d{4}-\d{2}-\d{2}$/.test(departureDate)) {
        return;
      }

      const relatedTour = tours.find(
        (tour) => String(tour.id || "").trim() === localTourId,
      );
      const isGlobalLinkedTour =
        relatedTour?.source_tag === "global" ||
        relatedTour?.source_tag === "global+local" ||
        String(relatedTour?.source_system || "")
          .trim()
          .toLowerCase() === "global-travel" ||
        Boolean(String(relatedTour?.source_tour_id || "").trim());

      if (!isGlobalLinkedTour) {
        return;
      }
      ...
      try {
        ...
        if (syncResult?.data?.status === "skipped" && syncResult.data.reason) {
          toast.warn(`Global seat sync skipped: ${syncResult.data.reason}`);
        }
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Global seat sync failed";
        toast.warn(`Global seat sync failed: ${message}`);
      }
    };
  ...
  const confirmedOrders = orders.filter((order) => {
    if (!["confirmed", "pending"].includes(order.status)) return false;

    if (providerScopeMode === "assigned") {
      return providerAssignedTourSet.has(String(order.tour_id ?? ""));
    }

    return !hasShowInProviderOrders || order.show_in_provider === true;
  });

  const visibleTours = tours.filter((tour) => {
    if (providerScopeMode === "assigned") {
      return providerAssignedTourSet.has(String(tour.id ?? ""));
    }

    return !hasShowInProviderTours || tour.show_in_provider;
  });
  ...
  const ordersPendingConfirmation = confirmedOrders.filter(
    (order) => !order.booking_confirmation,
  ).length;
  ...
  const providerScopeLabel =
    providerScopeMode === "assigned"
      ? `${providerAssignedTourIds.length} assigned tours`
      : "Legacy provider visibility";
  ...
  const workspaceStats = [
    ...
    },
    {
      ...
      value: ordersPendingConfirmation,
    },
    {
      label: "Visible Passengers",
      value: allPassengers.length,
    },
    {
      label: "Visible Tours",
      value: visibleTours.length,
    },
  ];

  const visibleSectionLabels = tabs.map((tab) => tab.label).join(", ");
  ...
  return (
    <div className="mono-shell">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="mono-container px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center justify-end gap-2">
          <button
      ...
      <div className="lg:hidden border-b border-gray-200 bg-white">
        <div className="mono-container px-4 sm:px-6 lg:px-8 py-3">
          <p className="mono-kicker">Sections</p>
      ...
      <div className="relative">
        <aside className="hidden lg:flex fixed left-0 top-14 z-30 w-72 h-[calc(100vh-3.5rem)] bg-white border-r border-gray-200 flex-col">
          <div className="p-5 border-b border-gray-200">
            <h2 className="mono-title text-lg">Sections</h2>
            <p className="text-xs text-gray-500 mt-1">Provider Interface</p>
          </div>
        ...
        <div className="lg:pl-72">
          <div className="w-full px-4 sm:px-6 lg:px-8 py-6 pb-16 sm:pb-20">
            <div className="mono-stack">
              {activeTab === "dashboard" && (
                <>
                  ...
                  <div className="mono-card p-5 sm:p-6">
                    <div className="grid gap-4 xl:grid-cols-[1.25fr,0.75fr]">
                      <div className="mono-stack-tight">
                        <p className="mono-kicker">Provider Workspace</p>
                        <h2 className="mono-title text-xl mt-1">
                          What you can do
                        </h2>
                        <p className="mono-subtitle text-sm mt-1">
                          Handle daily provider operations with a fixed flow:
                          Orders, Booking Confirmation, then Passenger Updates.
                        </p>

                        <div className="flex flex-wrap items-center gap-2 mt-2">
                          {readOnlyPreview && (
                            <span className="mono-badge">
                              Manager preview mode
                            </span>
                          )}
                          <span className="mono-badge">
                            {providerScopeLabel}
                          ...
                          <span className="mono-badge">
                            {ordersPendingConfirmation} pending confirmations
                          </span>
                        </div>

                        <p className="text-xs text-gray-500 mt-2">
                          Use the sections menu to move between{" "}
                  ...
                  <StatsCards orders={confirmedOrders} tours={visibleTours} />
                </>
              )}
                ...
                {providerScopeMode === "assigned" &&
                  providerAssignedTourIds.length === 0 && (
                    <div className="mono-card p-5 text-sm text-gray-500">
                      No tours are assigned to this provider yet.
                    </div>
                  )}
