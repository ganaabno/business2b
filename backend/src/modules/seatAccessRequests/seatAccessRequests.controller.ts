...
import {
  ...
  previewSeatAccessSerialSelectionService,
...
export const previewSeatAccessSerialSelectionController = asyncHandler(
  async (req: Request, res: Response) => {
    const input = parseSelectTourFromAccessInput(req.body);
    const data = await previewSeatAccessSerialSelectionService(
      req.user!,
      getParam(req.params.id),
      input,
    );
