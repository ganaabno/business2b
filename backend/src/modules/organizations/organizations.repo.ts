...
export const FALLBACK_ORGANIZATION_ID = "00000000-0000-0000-0000-000000000111";
const FALLBACK_ORGANIZATION_NAME = "Independent Requests";
const FALLBACK_ORGANIZATION_REGISTRATION = "SYS-UNBOUND-SEAT-REQUESTS-000111";
...
export async function ensureFallbackOrganizationRepo(params: {
  createdBy: string;
}) {
  const { rows } = await q<{
    id: string;
    ...
    values ($1::uuid, $2, $3, null, null, null, $4::uuid)
    on conflict (id)
    do update set name = public.organizations.name
    returning id::text, name, registration_number, merchant_code
    `,
    [
      FALLBACK_ORGANIZATION_ID,
      FALLBACK_ORGANIZATION_NAME,
      FALLBACK_ORGANIZATION_REGISTRATION,
      params.createdBy,
    ],
  );
